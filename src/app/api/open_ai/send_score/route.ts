...
import { sendScoreRequestSchema } from '@/app/lib/schemas';
...
export async function POST(request: NextRequest) {
  const parsed = await parseBody(request, sendScoreRequestSchema);
  if (parsed.error) return parsed.error;

  const { forecastPeriods, sport } = parsed.data;
  ...
  try {
    const aiResponse = await openai.chat.completions.create({
      ...
      messages: [
        ...
        {
          content: JSON.stringify(forecastPeriods),
          role: 'user',
        },
      ],
      model: 'gpt-4.1-nano',
