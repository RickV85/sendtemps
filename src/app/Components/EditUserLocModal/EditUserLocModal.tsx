...
export default function EditUserLocModal({
  ...
  const handlePatchRequest = (
    ...
    if (userLoc) {
      patchUserLocation(userLoc, patchType, userInput)
        .then((res) => {
          if (res.patchLoc.id && res.patchLoc.id === userLoc.id && userLocations) {
        ...
        .catch((error) => {
          console.error(error);
          setSubmitMsg('An error occurred while modifying location. Please try again.');
  ...
  const handleDeleteSubmit = () => {
    ...
    if (userLoc && userLoc.id) {
      deleteUserLocation(+userLoc.id, userLoc.user_id)
        ...
        .catch((error) => {
          console.error(error);
          setSubmitMsg('An error occurred while deleting location. Please try again.');
          resetErrorMsg(setSubmitMsg);
        });
    }
  };
  ...
  const createUserLocModalContent = (triggerId: string) => {
    ...
    if (userLocations && userLocations.length) {
      curLoc = userLocations?.find((loc) => loc?.id?.toString() === selectedUserLoc);
    }
    switch (triggerId) {
        ...
        return (
          <>
            ...
            <div className='modal-btn-div'>
              ...
              <button className='edit-user-loc-button' onClick={() => handleNameSubmit()}>
                Confirm
              </button>
            </div>
          </>
        ...
        return (
          <>
            ...
            <div className='modal-btn-div'>
              ...
              <button className='edit-user-loc-button' onClick={() => handleTypeSubmit()}>
                Confirm
              </button>
            </div>
          </>
        ...
        return (
          <>
            ...
            <div className='modal-btn-div'>
              ...
              <button className='edit-user-loc-button' onClick={() => handleDeleteSubmit()}>
                Confirm
              </button>
            </div>
          </>
        ...
        return (
          <>
            <h3 className='modal-heading'>
              Please scroll down to view entire map before adding a new location.
            </h3>
            <div className='modal-btn-div'>
              <button
                className='edit-user-loc-button'
